...
"
  ...
  if [ "${Domian_Mode}" == '3' -o "${dnsapi_flag}" == 'y' ] && [ ! -e ~/.acme.sh/acme.sh ]; then
    ...
    [ ! -e acme.sh-master.tar.gz ] && wget -qc https://github.com/acmesh-official/acme.sh/archive/refs/tags/3.0.2.tar.gz
...
EOF
  ...
  if [ "${rewrite}" == 'magento2' -a -e "config/${rewrite}.conf" ]; then
    ...
    if [ "${anti_hotlinking_flag}" == 'y' ]; then
      ...
      sed -i "s@^  root.*;@&\n      rewrite ^/ /403.html;@" ${web_install_dir}/conf/vhost/${domain}.conf
